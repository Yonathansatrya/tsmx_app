...
class _SalesOrderTrendBar extends StatelessWidget {
  final List<DocumentTrendPoint> points;
  final int selectedYear;
  final int selectedMonth;

  const _SalesOrderTrendBar({
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      selectedMonth == 0
                          ? 'Akumulasi Sales Order per bulan $selectedYear'
                          : 'Akumulasi Sales Order per minggu',
                      style: const TextStyle(
              ...
              _TrendBadge(value: delta, isUp: isUp),
            ],
          ),
          const SizedBox(height: 12),
          if (total <= 0)
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            value == 0
                ? 'Stabil'
                : '${isUp ? '+' : '-'}Rp ${formatErpCurrency(value.abs())}',
            style: TextStyle(
              color: color,
              fontSize: 10,
              fontWeight: FontWeight.w900,
...
class _TrendLinePainter extends CustomPainter {
  ...
  const _TrendLinePainter({required this.points, required this.maxValue});
...
class _EmptyTrend extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Text(
        'Belum ada Sales Order aktif pada periode ini.',
